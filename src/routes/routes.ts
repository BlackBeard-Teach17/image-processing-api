...
routes.get(
    ...
    async (req: express.Request, res: express.Response): Promise<unknown> => {
        ...
        if (imageValidator(filename, width, height) === false)
            // check if the user entered all the parameters and if the width and height are numbers
            return res
                .status(400)
                .send(
                    'Invalid parameters! Check that the parameters height and width are positive numbers',
                );
        ...
        if (checkFileExtension(fileformat) === false)
